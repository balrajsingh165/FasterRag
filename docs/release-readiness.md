...
**Assessment date: 2026-08-10 · Release assessed: the first public tagged beta (`v0.1.0-beta.1` + PyPI publication, TASK-0020/TASK-0087) · Confidence: 62/100 at that date, ~84/100 after the release-gating checklist below.**

> **Re-derived 2026-09-13: 68/100 for the beta; 15/100 as positioned.** Measured on `main` at 995eb84, before a batch of fifteen parallel workers that was halted without landing (TASK-0284). What moved since 2026-08-10: CI on `main` had been red since 2026-08-04 with nobody aware — the pre-commit hook passes locally and nothing read the Actions API — and is now 4 of 5 jobs green (`integration` needs a live backend); five production defects that passed every local run were fixed (alphabetical Redis eviction on 3.12/Windows, line-ending-sensitive CSV sniffing, a Linux-only mypy error, an HTTP/2 smuggling CVE, a stale lockfile); the release-gating engineering closes went from 2 of 8 to 5 of 8; the coverage gate from 83.13% failing to 86.31% passing; the suite from 2,060 to 2,335 tests. By dimension: engineering completeness 82, verification depth 78, truthfulness 90, operational readiness 65, performance evidence 15, release mechanics 45 — weighted ≈66, adjusted to 68 because every defect found was found by finally running things the way CI does, and that environment gap is now closed. **What holds it there, in order of weight**: the license decision (TASK-0164, irreversible on first upload), the identity-encoding decision (TASK-0210, frozen by the tag), the `integration` CI job, the reranker default (TASK-0175), the D7 gate's drifted baseline (TASK-0244), four of six advertised adapters absent, and zero citable benchmark entries — the last being the ceiling on the *positioned* score, not the beta. The 15 is unchanged from the previous re-derivation and will stay there until a benchmark is recorded on isolated hardware (B4).
