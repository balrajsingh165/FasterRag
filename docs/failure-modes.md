...
| 33 | Disk | Disk full during ingest | OS write error → typed `IngestionError` (**specified**; measured, the raw `OSError` still escapes — TASK-0234); disk gauge alarm | Clean halt at a checkpoint; queues drained; no partial index writes | Free space; job resumes from checkpoint | Chaos: disk-full scenario, plus the real size-limited-filesystem variant |
