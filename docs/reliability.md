...
| **Circuit breakers** | Per provider (LLM, embeddings, vector DB); open after `failure_threshold` consecutive failures; half-open probes after `reset_timeout_ms`; **state exported as the `fasterrag_circuit_state` metric**. Only **retryable** failures count: a 401 fails identically forever, so counting it would open the breaker, half-open it after the timeout, fail again, and re-open — presenting a permanent misconfiguration as an intermittent outage. Wired into the embedding pool's retry loop, and checked *inside* it so a breaker that opens mid-batch stops the remaining attempts rather than spending the whole retry budget on a provider already known dead. | `reliability.circuit_breaker.*` |
...
Specified in full in [testing-strategy.md](testing-strategy.md): unit (≥ 85% coverage on `core/`, `adapters/`, `workers/`) · property-based chunker invariants (Hypothesis) · golden-file parser tests · testcontainers integration (Qdrant docker + remote modes) · the shared adapter contract suite · load (k6/Locust, hardware-annotated) · 24 h soak (no memory/fd growth) · the chaos suite (D12) · mutation testing (mutmut) on chunk-boundary and retrieval-scoring logic.
