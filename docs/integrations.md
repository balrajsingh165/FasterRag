...
| OpenTelemetry | `observability.otel: true` | Spans shipped — the four RAG spans export over OTLP/HTTP to `otel_endpoint`, preserving the fasterRag trace id. Metrics are not yet exported (TASK-0189). Needs the `otel` extra | Exports spans via OTLP to `otel_endpoint` | your collector |
