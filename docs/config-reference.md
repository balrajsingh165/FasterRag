...
| `embeddings.dimensions` | int\|null | `null` | ≥ 8 when set | Output dimensionality; `null` = the model's native size, which is the right setting unless you have a reason. Behaviour differs by provider, because shortening a vector is only lossless for a model trained for it (Matryoshka representation learning). **Hosted providers that support it** (OpenAI `text-embedding-3-*`) receive the value as an API parameter and shorten server-side. **Local `huggingface` models** cannot be shortened, so a value that disagrees with what the model emits is refused at load time with an error naming both sizes — rather than left to fail later as a rejected upsert against a collection already created at the wrong width. Must match the collection's vector size. |
