...
| Scanned PDFs index with no useful text | OCR yielded nothing; the chunk carries a `low_text_yield` parse flag rather than silently indexing emptiness | Check the flag in document metadata; verify the OCR path is available for that source type |
