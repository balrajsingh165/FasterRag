...
- [ ] TASK-0180: Finish tenant-tagging the remaining state: collection operations (so a tenant cannot list or drop another's), the trace store (a trace records the query text and the retrieved chunks), and metric labels (`fasterrag_requests_total` already declares a `tenant` label that always reads `none`). Queries and the semantic cache are scoped as of TASK-0179; until this lands, storage-level separation means one collection per tenant
...
- [ ] TASK-0176: `normalise_source` (TASK-0141) changed the document id of every path-based source, so collections indexed before it no longer match ids computed after it. Observed: the `handbook_baseline` collection scored a flat 0.0 until re-ingested, which reads as total retrieval failure rather than an id-scheme change. Needs a documented migration note in `disaster-recovery.md` and `data-model.md`, and ideally a detectable marker so a stale collection reports itself instead of scoring zero
