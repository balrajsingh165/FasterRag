...
- [ ] TASK-0179: Multi-tenancy isolation, the remaining half of the security layer. Keys already parse an optional tenant and `AuthMiddleware` puts the resolved key on the request scope, so the credential side is done; what is missing is enforcement at the service layer — every collection operation, query, cache entry, trace, and metric tenant-tagged, cross-tenant access refused with `TENANT_FORBIDDEN` regardless of endpoint, and semantic-cache entries scoped so a hit can never return another tenant's answer. `security.multi_tenancy` refuses to start until this lands
