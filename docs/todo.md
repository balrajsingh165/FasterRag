...
- [ ] TASK-0177: The markdown parser appears to decode UTF-8 source files using the platform codepage: an em-dash written as `â` came back from the index as `â€”`. The file on disk is valid UTF-8 and the config loader already handles this correctly with `utf-8-sig`, so the defect is in the parse path. Reproduce with any non-ASCII corpus and fix the decode, then re-ingest the affected fixtures
