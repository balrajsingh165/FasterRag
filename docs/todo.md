...
- [ ] TASK-0142: The handbook baseline is a smoke gate, not a discriminating one: 6 documents scored at k=5 means a retriever returns five of six, so recall@5 is close to free. MRR carries the signal (1.0 = correct document ranked first for all 12 queries). A baseline that detects *small* regressions needs a larger fixture with topically overlapping documents
