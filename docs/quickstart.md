...
`doctor` checks Docker, free ports (including **both 6333 and 6334** for Qdrant), disk, RAM/GPU, backend reachability, key validity, and config validity. **Every failed check prints a concrete fix.** It must pass before any provisioning runs — this is what makes one-toggle provisioning survivable on an arbitrary machine (D10). `fasterrag doctor --fix` additionally repairs the two failures that can be repaired safely — a missing named storage volume, and fasterRag's own stopped container — then re-runs every check and reports what is true afterwards; everything else is reported as needing a human, with the command to run. It never frees a port, deletes files, or writes secrets, and it still exits `4` if anything is left failing ([cli-reference.md](cli-reference.md#doctor---fix)).
